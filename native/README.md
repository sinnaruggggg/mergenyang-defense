...
- v0.2.3: `art/backgrounds-tall-v1`의 승인된 긴 배경 4종 사용. 폭 기준 균등 배율로 중앙 배치하고 기기 높이에 따라 위아래가 더 보임. 가장자리 거울 반복·세로 늘이기는 제거함.
- 로비: 상단 HUD는 화면 위에, 대장장이와 하단 카드 묶음은 화면 아래에 붙음.
- 다른 메뉴: 상단 재화 바·제목은 위, 하단 탭은 아래에 붙음(`anchorTop`/`anchorBottom`).
- 전투: 레인·보드 크기는 유지하고 긴 배경을 사용. 상단 HUD와 일시정지·배속 버튼은 화면 위에 붙음.
- 원화 비율(약 9:21)보다 긴 기기는 전체 이미지를 가로세로 동일 비율로 확대해 덮으므로 좌우 일부가 잘릴 수 있음.
